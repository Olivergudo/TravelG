...
import { ChevronLeft, Clipboard, FileText, LoaderCircle, MoreHorizontal, Pencil, ReceiptText, Search, Share2, ShoppingBasket, Trash2, X } from "lucide-react";
import { useMemo, useState } from "react";
import { categoryEmoji, CategoryManager, QuickExpenseForm } from "./expense-ui";
...
import { movementAsText, movementPdf, movementPdfFilename, type MovementShareData } from "@/lib/movements/share";
...
function HistoryDetail({ item, currency, close }: { item: PurchaseHistoryItem; currency: Currency; close: () => void }) {
  const [shareOpen, setShareOpen] = useState(false);
  const shareData: MovementShareData = { title: item.title, amount: item.amount, date: item.date, category: item.category?.name || "Otros", description: item.description, products: item.products, isTicket: Boolean(item.purchase) };
  return <><div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/50 sm:items-center"><section className="theme-card max-h-[calc(100dvh-env(safe-area-inset-top))] w-full max-w-lg overflow-y-auto rounded-t-[30px] bg-white p-5 safe-bottom sm:rounded-[30px]"><div className="flex items-center gap-2"><button onClick={close} aria-label="Volver" className="grid h-11 w-11 place-items-center rounded-full bg-[#edf2ee]"><ChevronLeft size={21}/></button><p className="flex-1 text-center text-xs font-bold uppercase tracking-[.16em] text-[#718078]">{item.purchase ? "Detalle de compra" : "Detalle"}</p><button onClick={close} aria-label="Cerrar" className="grid h-11 w-11 place-items-center rounded-full bg-[#edf2ee]"><X size={20}/></button></div><div className="mt-5 text-center"><span style={{ backgroundColor: getCategorySoftColor(item.category) }} className="mx-auto grid h-14 w-14 place-items-center rounded-2xl text-2xl">{categoryEmoji(item.category)}</span><h2 className="mx-auto mt-3 max-w-sm text-2xl font-bold leading-tight">{item.title}</h2></div><dl className="mt-6 grid grid-cols-2 gap-3"><Info label="Monto" value={formatCurrency(item.amount, currency)}/><Info label="Categoría" value={item.category?.name || "Otros"}/><div className="col-span-2"><Info label="Fecha" value={formatLongDate(item.date)}/></div></dl>{item.description && <section className="mt-5"><h3 className="text-xs font-bold uppercase tracking-[.14em] text-[#718078]">Descripción</h3><p className="mt-2 text-sm">{item.description}</p></section>}{item.products.length > 0 && <section className="mt-6"><div className="flex items-center gap-2"><ReceiptText className="text-[#176b46]" size={19}/><h3 className="text-xs font-bold uppercase tracking-[.14em] text-[#718078]">Productos · {item.products.length}</h3></div><div className="mt-2 overflow-hidden rounded-2xl border border-black/[.07]">{item.products.map((p) => <div key={p.id} className="flex gap-3 border-b border-black/[.06] px-4 py-3 text-sm last:border-0"><span className="min-w-0 flex-1 font-semibold">{p.productName}</span><span className="shrink-0 text-[#718078]">{p.quantity} × {p.unitPrice > 0 ? formatCurrency(p.unitPrice, currency) : "Sin precio"}</span></div>)}</div></section>}<button onClick={() => setShareOpen(true)} className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#176b46] font-semibold text-white"><Share2 size={20}/> Compartir</button></section></div>{shareOpen && <MovementShareSheet item={shareData} currency={currency} close={() => setShareOpen(false)}/>}</>;
}
...
function MovementShareSheet({ item, currency, close }: { item: MovementShareData; currency: Currency; close: () => void }) {
  ...
  const createPdf = async () => {
    ...
    try {
      ...
      const result = await shareOrDownloadPdf(blob, movementPdfFilename(item), item.title);
      setStatus(result === "shared" ? "PDF compartido" : "PDF descargado");
    } catch (cause) {
      if (!(cause instanceof DOMException && cause.name === "AbortError")) setStatus("No pudimos crear el PDF. Intenta nuevamente.");
    } finally { setWorking(false); }
  };
  const copy = async () => {
    try { await navigator.clipboard.writeText(movementAsText(item, currency)); setStatus("Resumen copiado"); }
    catch { setStatus("No pudimos copiar el resumen."); }
  };
  return <div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/55 sm:items-center" onClick={close}><section className="theme-card w-full max-w-lg rounded-t-[30px] bg-white p-5 safe-bottom sm:rounded-[30px]" onClick={(event) => event.stopPropagation()}><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#176b46]">Compartir</p><h2 className="mt-1 truncate text-xl font-bold">{item.title}</h2></div><button onClick={close} aria-label="Cerrar" className="grid h-11 w-11 place-items-center rounded-full bg-[#edf2ee]"><X/></button></div><div className="mt-5 space-y-2"><button disabled={working} onClick={createPdf} className="flex min-h-14 w-full items-center gap-3 rounded-2xl bg-[#176b46] px-4 font-semibold text-white disabled:opacity-60">{working ? <LoaderCircle className="animate-spin" size={20}/> : <FileText size={20}/>} {working ? "Preparando PDF..." : "Compartir como PDF"}</button><button disabled={working} onClick={copy} className="flex min-h-14 w-full items-center gap-3 rounded-2xl bg-[#e6f3ec] px-4 font-semibold text-[#176b46]"><Clipboard size={20}/> Copiar resumen</button></div>{status && <p role="status" className={`mt-3 text-center text-sm font-semibold ${status.startsWith("No pudimos") ? "text-red-700" : "text-[#176b46]"}`}>{status}</p>}</section></div>;
}
