...
function HistoryDetail({ item, currency, close }: { item: PurchaseHistoryItem; currency: Currency; close: () => void }) {
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/50 sm:items-center"><section className="theme-card max-h-[calc(100dvh-env(safe-area-inset-top))] w-full max-w-lg overflow-y-auto rounded-t-[30px] bg-white p-5 safe-bottom sm:rounded-[30px]"><div className="flex items-center gap-2"><button onClick={close} aria-label="Volver" className="grid h-11 w-11 place-items-center rounded-full bg-[#edf2ee]"><ChevronLeft size={21}/></button><p className="flex-1 text-center text-xs font-bold uppercase tracking-[.16em] text-[#718078]">{item.purchase ? "Detalle de compra" : "Detalle"}</p><button onClick={close} aria-label="Cerrar" className="grid h-11 w-11 place-items-center rounded-full bg-[#edf2ee]"><X size={20}/></button></div><div className="mt-5 text-center"><span style={{ backgroundColor: getCategorySoftColor(item.category) }} className="mx-auto grid h-14 w-14 place-items-center rounded-2xl text-2xl">{categoryEmoji(item.category)}</span><h2 className="mx-auto mt-3 max-w-sm text-2xl font-bold leading-tight">{item.title}</h2></div><dl className="mt-6 grid grid-cols-2 gap-3"><Info label="Monto" value={formatCurrency(item.amount, currency)}/><Info label="Categoría" value={item.category?.name || "Otros"}/><div className="col-span-2"><Info label="Fecha" value={longDate(item.date)}/></div></dl>{item.description && <section className="mt-5"><h3 className="text-xs font-bold uppercase tracking-[.14em] text-[#718078]">Descripción</h3><p className="mt-2 text-sm">{item.description}</p></section>}{item.products.length > 0 && <section className="mt-6"><div className="flex items-center gap-2"><ReceiptText className="text-[#176b46]" size={19}/><h3 className="text-xs font-bold uppercase tracking-[.14em] text-[#718078]">Productos · {item.products.length}</h3></div><div className="mt-2 overflow-hidden rounded-2xl border border-black/[.07]">{item.products.map((p) => <div key={p.id} className="flex gap-3 border-b border-black/[.06] px-4 py-3 text-sm last:border-0"><span className="min-w-0 flex-1 font-semibold">{p.productName}</span><span className="shrink-0 text-[#718078]">{p.quantity} × {formatCurrency(p.unitPrice, currency)}</span></div>)}</div></section>}</section></div>;
}
