...
export async function authenticatedSupabase(request: Request) {
  ...
  const { data: profile } = await client.from("profiles").select("plan,pro_expires_at").eq("id", data.user.id).maybeSingle();
  const pro = profile?.plan === "pro" && (!profile.pro_expires_at || new Date(profile.pro_expires_at).getTime() > Date.now());
  return { client, user: data.user, pro };
}
